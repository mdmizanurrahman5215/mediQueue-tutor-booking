...
export default function TutorDetailsPage({ id }) {
  // 1. Params specific route/id extract
  ...
  // 2. MongoDB/Backend API call using useEffect
  useEffect(() => {
    if (!id) return;
    ...
    const fetchTutorDetails = async () => {
      try {
        setLoading(true);
        // আপনার ব্যাকএন্ড API এর URL এখানে বসান
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 py-10 transition-colors">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Left / Top Section: Main Profile Card */}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{tutorData.experience}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm font-semibold text-gray-800 dark:text-gray-200">{tutorData.location}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 sm:p-8 border border-gray-100 dark:border-gray-800 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-1">
                  ...
                  <p className="text-sm font-bold text-gray-900 dark:text-white">{tutorData.availableDays}</p>
                  <p className="text-xs text-gray-600 dark:text-gray-400">{tutorData.availableTimeSlot}</p>
                </div>
                ...
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-1">
                  ...
                  <p className="text-sm font-bold text-gray-900 dark:text-white">
                    {tutorData.sessionStartDate ? new Date(tutorData.sessionStartDate).toLocaleDateString(undefined, {
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric'
                    }) : 'N/A'}
                  </p>
                  <p className="text-xs text-green-600 dark:text-green-400 font-semibold">Immediate Enrollment</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white dark:bg-gray-900 rounded-3xl p-6 border border-gray-100 dark:border-gray-800 shadow-sm space-y-6 sticky top-6">
              ...
              <div className="space-y-1 text-center sm:text-left">
                ...
                <div className="flex items-baseline justify-center sm:justify-start space-x-1">
                  <span className="text-3xl font-extrabold text-gray-900 dark:text-white">৳{tutorData.hourlyFee}</span>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 space-y-3">
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className={`font-bold px-2.5 py-0.5 rounded-full text-xs ${
                    tutorData.totalSlot > 0 ? 'bg-green-100 text-green-700 dark:bg-green-950/60 dark:text-green-400' : 'bg-red-100 text-red-700'
                  }`}>
                ...
                <div className="flex justify-between items-center text-sm">
                  ...
                  <span className="font-semibold text-gray-900 dark:text-white">{tutorData.teachingMode}</span>
                </div>
              </div>
              ...
              >
                {tutorData.totalSlot > 0 ? 'Book Session Now' : 'Slots Unavailable'}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
          <div className="bg-white dark:bg-gray-900 rounded-3xl max-w-md w-full p-6 space-y-6 border border-gray-100 dark:border-gray-800 shadow-xl relative">
            ...
            <div className="flex justify-between items-center border-b border-gray-100 dark:border-gray-800 pb-4">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">Confirm Session Booking</h3>
              <button 
            ...
            {bookingSuccess ? (
              <div className="text-center py-8 space-y-3">
                ...
                <h4 className="text-xl font-bold text-gray-900 dark:text-white">Booking Confirmed!</h4>
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  Your tutoring session with {tutorData.tutorName} has been booked.
                </p>
              </div>
            ) : (
              /* Modal Form */
              <form onSubmit={handleBookingSubmit} className="space-y-4">
                <div className="p-4 bg-gray-50 dark:bg-gray-800/50 rounded-2xl space-y-2 text-xs text-gray-600 dark:text-gray-300">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold text-gray-900 dark:text-white">{tutorData.tutorName}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Subject:</span>
                    <span className="font-semibold text-gray-900 dark:text-white">{tutorData.subject}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Fee:</span>
                    <span className="font-bold text-blue-600 dark:text-blue-400">৳{tutorData.hourlyFee} / hr</span>
                  </div>
                </div>

                <div className="space-y-1">
                  <label className="text-xs font-semibold text-gray-700 dark:text-gray-300">Student Name</label>
                  <input
                    type="text"
                    required
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm transition-colors shadow-sm mt-2"
                >
